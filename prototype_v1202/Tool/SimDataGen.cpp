...
SimDataGen::SimDataGen(SIMRE_LIB_PARAM param){
	...
	px[0] = 0.0f;	px[1] = 0.0f;  px[2] = 0.0f;  px[3] = -1.0f;	px[4] = 0.0f;
	py[0] = 0.0f;	py[1] = 1.0f;  py[2] = -1.0f;  py[3] = 0.0f;	py[4] = 0.0f;
...
void SimDataGen::GetTgtRange(float x, float y, float w, float R, float v, float a, 
	...
	for(int i=0; i<count; i++) {
		...
		fai = w*ts;
		rge[i] = (float)(R + (v*ts + 0.5*a*ts*ts + y*cos(fai) - x*sin(fai))/1000.0f);
	}
}
...
void SimDataGen::GenData(){
	...
	printf("file open\n");
	printf("file writing...\n");
	printf("共%d行数据\n", m_count*m_num*sample); 

	for(int s=0; s<m_count*m_num*sample; s++)  // 
	{
