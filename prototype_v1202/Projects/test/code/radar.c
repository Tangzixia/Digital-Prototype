...
int main()
{
    ...
    fftw_complex *fdpc_in_temp = (fftw_complex *)malloc(sizeof(fftw_complex) * SampleNumber);
    fftw_complex *fdpc_out_temp = (fftw_complex *)malloc(sizeof(fftw_complex) * SampleNumber);
    fftw_plan fdpc_plan;
    ...
    for (i = 0; i < number; i++)
    {
        ...
        readData("./test.txt", echo, i * SampleNumber * PulseNumber, 0);
        gettimeofday(&t1, NULL);
        PC_NCA_AJC(echo, pRcb, uRow, uCol, uRange, bNcaEn, pNcaMag);
        // writeData("./pna.txt",echo);
        NarrowPulseJam(echo,PulseNumber,SampleNumber,SampleNumber);
        // writeData("./npj.txt",echo);
        frequencyDomainPulseCompression(echo, echo_pulse, echo_fft, coeff_fft, pc0, pc, pc_fft, echo_plan, pc_plan);
        ...
        writeData_1("./result.txt", cfar_result);
